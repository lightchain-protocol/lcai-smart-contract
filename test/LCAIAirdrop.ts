...
describe("LCAIAirdrop", function () {
  ...
  describe("Fee Withdrawal", function () {
    ...
    it("Should allow owner to withdraw all fees", async function () {
      ...
      const tx = await airdrop.withdrawAllFees(owner.address);
      const receipt = await tx.wait();
      const gasUsed = receipt!.gasUsed * receipt!.gasPrice;
    ...
    it("Should revert if non-owner tries to withdraw fees", async function () {
      ...
      await expect(
        airdrop.connect(buyer1).withdrawFees(buyer1.address, claimFee)
      ).to.be.revertedWithCustomError(airdrop, "OwnableUnauthorizedAccount");
    });
    ...
    it("Should revert if withdrawal amount is zero", async function () {
      ...
      await expect(airdrop.withdrawFees(owner.address, 0n)).to.be.revertedWith(
        "LCAIAirdrop: Amount must be greater than 0"
      );
    });
    ...
    it("Should revert if recipient address is zero", async function () {
      ...
      await expect(
        airdrop.withdrawFees(ethers.ZeroAddress, claimFee)
      ).to.be.revertedWith("LCAIAirdrop: Invalid recipient address");
    });
    ...
    it("Should revert if withdrawal amount exceeds balance", async function () {
      ...
      await expect(
        airdrop.withdrawFees(owner.address, parseEther("100"))
      ).to.be.revertedWith("LCAIAirdrop: Insufficient ETH balance");
    });

    it("Should revert when withdrawing all fees if no fees collected", async function () {
      const { airdrop } = await deployFixture();

      // Don't collect any fees

      await expect(airdrop.withdrawAllFees(owner.address)).to.be.revertedWith(
        "LCAIAirdrop: No fees to withdraw"
      );
    });

    it("Should revert if withdrawAllFees recipient is zero address", async function () {
      const { airdrop } = await deployFixture();

      await expect(
        airdrop.withdrawAllFees(ethers.ZeroAddress)
      ).to.be.revertedWith("LCAIAirdrop: Invalid recipient address");
    });
  });
});
