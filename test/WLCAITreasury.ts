...
describe("WLCAITreasury", function () {
  // ===== HELPER FUNCTIONS =====

  async function deployWLCAITreasuryContracts() {
    ...
    const treasury = await ethers.deployContract("WLCAITreasury", [
  ...
  it("Should deploy with correct initial state", async function () {
    const { treasury, adminContract } = await deployWLCAITreasuryContracts();
  ...
  it("Should reject deployment with EOA admin", async function () {
    ...
    await expect(
      ethers.deployContract("WLCAITreasury", [
        deployer.address,
        admin.address, // EOA instead of contract
      ])
    ).to.be.revertedWithCustomError(await ethers.getContractFactory("WLCAITreasury"), "AdminMustBeMultisig");
  });

  it("Should receive ETH via receive function", async function () {
    const { treasury } = await deployWLCAITreasuryContracts();
  ...
  it("Should allow owner to transfer ETH", async function () {
    ...
    await expect(
      treasury.connect(deployer).transferETH(recipient1.address, parseEther("5"))
    )
  ...
  it("Should prevent non-owner from transferring ETH", async function () {
    ...
    await expect(
      treasury.connect(nonAdmin).transferETH(recipient1.address, parseEther("5"))
  ...
  it("Should revert on insufficient ETH balance", async function () {
    ...
    await expect(
      treasury.connect(deployer).transferETH(recipient1.address, parseEther("10"))
  ...
  it("Should allow deposit of ERC20 tokens", async function () {
    const { treasury, token } = await deployWLCAITreasuryContracts();

    // Approve treasury to spend tokens
    await token.connect(deployer).approve(await treasury.getAddress(), parseEther("100"));
    ...
    await expect(
      treasury.connect(deployer).deposit(await token.getAddress(), { value: parseEther("50") })
    )
      .to.emit(treasury, "Deposit")
  ...
  it("Should allow owner to transfer ERC20 tokens", async function () {
    ...
    await expect(
      treasury.connect(deployer).transferERC20(await token.getAddress(), recipient1.address, parseEther("50"))
    )
    ...
    expect(await treasury.spent(await token.getAddress())).to.equal(parseEther("50"));
  });
  ...
  it("Should prevent non-owner from transferring ERC20 tokens", async function () {
    ...
    await expect(
      treasury.connect(nonAdmin).transferERC20(await token.getAddress(), recipient1.address, parseEther("50"))
    ).to.be.revertedWithCustomError(treasury, "OwnableUnauthorizedAccount");
  });

  it("Should prevent ERC20 transfer when paused", async function () {
    const { treasury, adminContract, token } = await deployWLCAITreasuryContracts();
    await fundTreasuryERC20(treasury, token, "100");
  ...
  it("Should revert on insufficient ERC20 balance", async function () {
    ...
    await expect(
      treasury.connect(deployer).transferERC20(await token.getAddress(), recipient1.address, parseEther("100"))
    ).to.be.revertedWithCustomError(treasury, "InsufficientBalance");
  });
  ...
  it("Should allow ETH transfers to whitelisted addresses when whitelist is enabled", async function () {
    ...
    await treasury.connect(deployer).transferETH(recipient1.address, parseEther("5"));
  ...
  it("Should allow ERC20 transfers to whitelisted addresses when whitelist is enabled", async function () {
    ...
    // Transfer should succeed
    await treasury.connect(deployer).transferERC20(await token.getAddress(), recipient1.address, parseEther("50"));
  ...
  it("Should track ETH spent correctly across multiple transfers", async function () {
    ...
    // Transfer 1
    await treasury.connect(deployer).transferETH(recipient1.address, parseEther("10"));
    expect(await treasury.spent(ethers.ZeroAddress)).to.equal(parseEther("10"));

    // Transfer 2
    await treasury.connect(deployer).transferETH(recipient2.address, parseEther("25"));
    expect(await treasury.spent(ethers.ZeroAddress)).to.equal(parseEther("35"));

    // Transfer 3
    await treasury.connect(deployer).transferETH(recipient3.address, parseEther("15"));
  ...
  it("Should track ERC20 spent correctly across multiple transfers", async function () {
    ...
    // Transfer 1
    await treasury.connect(deployer).transferERC20(tokenAddress, recipient1.address, parseEther("30"));
    expect(await treasury.spent(tokenAddress)).to.equal(parseEther("30"));

    // Transfer 2
    await treasury.connect(deployer).transferERC20(tokenAddress, recipient2.address, parseEther("40"));
    expect(await treasury.spent(tokenAddress)).to.equal(parseEther("70"));

    // Transfer 3
    await treasury.connect(deployer).transferERC20(tokenAddress, recipient3.address, parseEther("20"));
  ...
  it("Should handle both ETH and ERC20 independently", async function () {
    ...
    expect(await treasury.getETHBalance()).to.equal(parseEther("50"));
    expect(await treasury.getBalance(await token.getAddress())).to.equal(parseEther("100"));
    ...
    // Transfer ERC20
    await treasury.connect(deployer).transferERC20(await token.getAddress(), recipient2.address, parseEther("20"));
    expect(await treasury.spent(await token.getAddress())).to.equal(parseEther("20"));

    // Check both balances are correctly updated
    expect(await treasury.getETHBalance()).to.equal(parseEther("40"));
    expect(await treasury.getBalance(await token.getAddress())).to.equal(parseEther("80"));
  });
  ...
  it("Should work with both whitelist and blacklist disabled", async function () {
    ...
    await treasury.connect(deployer).transferERC20(await token.getAddress(), recipient2.address, parseEther("25"));
    expect(await treasury.spent(await token.getAddress())).to.equal(parseEther("25"));
  });
});
