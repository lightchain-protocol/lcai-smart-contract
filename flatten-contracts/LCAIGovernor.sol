...
library SignatureChecker {
    ...
    function isValidSignatureNow(
        ...
        if (signer.length < 20) {
            ...
            return
                isValidSignatureNow(address(bytes20(signer)), hash, signature);
        } else {
            (bool success, bytes memory result) = address(bytes20(signer))
                .staticcall(
                    abi.encodeCall(
                        IERC7913SignatureVerifier.verify,
                        (signer.slice(20), hash, signature)
                    )
                );
            return (success &&
                result.length >= 32 &&
                abi.decode(result, (bytes32)) ==
                    bytes32(IERC7913SignatureVerifier.verify.selector));
        }
    }
        ...
        return
            ...
            interfaceId ==
                type(IGovernor).interfaceId ^
                    IGovernor.getProposalId.selector ||
